...
function shareSelectedDateOnWhatsApp(){
  ...
  if(!list.length){
    toast('No events found for selected date.',true);
    return;
  }

  const title=new Date(d+'T00:00:00').toLocaleDateString('en-GB',{
    weekday:'long',
    day:'2-digit',
    month:'long',
    year:'numeric'
  ...
  let msg=`TELEC Event Schedule\nDate: ${title}\n\n`;

  list.forEach((e,i)=>{
    msg+=`--------------------------------------------------\n\n`;
    msg+=`${i+1}. ${e.familyPersonName || 'Event'}\n\n`;
    msg+=`Time:\n${fmtTime(e.eventTime) || '-'}\n\n`;
    msg+=`Venue:\n${e.venueLocation || '-'}\n\n`;
    msg+=`City:\n${e.city || '-'}\n\n`;
    ...
    if(e.googleMapsLink && e.googleMapsLink.trim()){
      msg+=`Google Maps:\n${e.googleMapsLink.trim()}\n\n`;
    }
  });

  msg+=`--------------------------------------------------\n`;
  msg+=`Generated by TELEC Smart Event Manager`;
